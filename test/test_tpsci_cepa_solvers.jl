...
@testset "tpsci cepa solvers" begin
    ...
    # ── one solver pass for all roots vs one pass per root ──────────────────────
    ...
    @test isapprox(run_exact(solver=:minres, build_hqq=:fois, block_roots=false),
                   run_exact(solver=:minres, build_hqq=:sparse, block_roots=false), atol=1e-8)
